(function() {
  ...
  function WanSelect() {
    ...
    var template = ['<div class="wan-select" outside-click="vm.hideSelect()">',
      ...
      '      <md-button class="md-raised ws-select-button" ng-click="vm.selectToggle()">Unselected</md-button>',
      ...
      '      <span ng-show="vm.dataUnselected.length === 0" class="ws-none-select">No items to choose from </span>',
    ...
    function Controller($scope) {
      ...
      vm.setSearchResult = function() {
        ...
        if (length === 1) {
          ...
          vm.result = 'Selected all';
        } else {
          vm.result = 'Selected ('+length+')';
        }
      };
